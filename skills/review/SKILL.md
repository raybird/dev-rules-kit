...
- 核對每項驗收標準的紅綠重構證據
- 形式判定依 `docs/AGENTS.md`「狀態判定的作用域是核准表」以表頭為準：有 `| Scenario | 核准日期 | 狀態 |` 即逐項表格，摘要句不改變判定。核准表為單行省略形式時，另掃 README 其他區塊確認沒有未核准的 Scenario：掃到 `待核准` / `待重新核准` 時要求展開為逐項表格再依表判定，列為 MUST FIX（單行會讓核准查核靜默通過）。核准表為逐項表格時狀態以表為準，敘述區塊的同名字串屬時序記錄，不列為缺失
