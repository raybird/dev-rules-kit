...
- 補上核准表形式判定的可執行判準（以 `| Scenario | 核准日期 | 狀態 |` 表頭為準）。v2.19.0 的規範文字正確但沒給 agent 判定方法，混合形態（摘要句加逐項表）會被誤判為單行形式，要求它展開一份本來就有的表——line-oa-plus `issue-0242` 是實例。判定改用表頭而非 Scenario ID 前綴：規範從未定義 ID 格式，掃前綴會漏掉採其他命名的下游。
- 記錄下游掃描結果：164 份 issue 中真正採單行省略形式者 7 份、誤用 0 份，WF-06 對照組因此無真實樣本、只能人工合成；`issue-0242` 可作正向案例的現成樣本。

## [2.19.0] - 2026-09-10

### 修正（`docs/AGENTS.md` 1.21、`dev-cycle` / `create-pr` / `review` — **下游需重新複製規範與這三支 skill**）

- 明訂核准狀態的判定作用域是核准表：Timeline 與其他敘述區塊出現 `待核准` / `待重新核准` 屬時序記錄，不再讓已全數核准的 issue 被判成尚有未核准項。原描述把 gate 寫成字串殘留掃描，與同一份規範的時序保留原則互斥，實作者被迫挑選措辭繞開 gate（line-oa-plus issue-0276 實跑回報）。
- 單行省略形式沒有狀態欄可讀，保留全檔形式查核並改為要求展開為逐項表格再依表判定，維持 1.12 版「分批時收斂成單行會靜默通過」的守門缺口不被重新打開。
- 新增 WF-06 回歸案例涵蓋上述兩條路徑，待實跑。

## [2.18.0] - 2026-09-09

### 修正（`scripts/`、`rules/`、使用文件與 CI）

- 修正重複安裝技能時多出 `<name>/<name>/SKILL.md`、原位置未更新的錯誤。更新同名檔案並保留其他檔案；既有巢狀副本需確認內容後手動清理。
- 新增 `scripts/init-project.py`，部署下游的 `docs/AGENTS.md`、`docs/agents/` 與 `docs/_templates/`，支援 dry-run 與內容一致性檢查；遇到客製內容或路徑衝突時先整批停止，保留既有文件。
- 統一雙語規則的 Small 輕量驗收形式、核准紀錄與測試步驟，移除後續步驟無條件要求 Gherkin 的矛盾。下游需同步更新雙語規則中使用的版本。
- 新增六項安裝與初始化回歸測試並納入 CI；以暫存副本恢復舊版複製方式，確認更新測試會失敗。
- 修正 README 技能數量與清單、CLAUDE.md 驗證指引，補上專案初始化步驟；新增五個 agent 流程回歸案例，保留待實跑狀態。

## [2.17.0] - 2026-09-07

### 變更（`docs/AGENTS.md` 1.20、`docs/agents/`、全部 7 支 skill 及對應 workflow — **下游需重新複製規範與這些 skill**）

- **Large 的兩份分析文件從「一律建立」改為依觸發條件建立**：`requirement-analysis.md` 對應「需求範圍或現況行為有需要調查才寫得下來的部分」，`technical-analysis.md` 對應「澄清過程提出了兩個以上方案並做了取捨」。Large 的必建集合縮為 `README.md` 與 `implementation-plan.md`。

  這是下游實跑資料推動的。原規則與同一份規範的 YAGNI 原則直接衝突，而實作者已經在援引後者論證不建——`issue-0188` 寫「補寫三份事前分析文件會違反同一份規範的 YAGNI 原則」，`issue-0167` 寫「重抄一份只會製造漂移」。兩條規則打架時，明文的那條輸了。

  **保留而非刪除這兩份文件**，同樣是資料決定的：23 份 requirement-analysis 建立後的 11 次修改裡有 9 次是實質內容變更，其中兩次是文件推翻自己原本的判斷。原本的方向是刪除，被這份資料否決。

  未一併放寬 `implementation-plan.md`——它有實際消費者（`execute-task` 讀其實作步驟、`decompose` 以其為輸入）。

- **`**分級**` 欄位改為取開頭的詞判定**，值後面可接補充說明。下游有 4 份 README 寫成 `Large（暫定）` 這類形式，嚴格比對會讓它們落進檔案形狀回推路徑。同段的「唯一依據」改述為「唯一可靠來源」並說明原因：分析文件變成選用後，Large 未建分析文件時的形狀與 Medium 相同，回推無從區分。

- **`dev-cycle` 的形狀回推得到 Medium 或 Small 時，先向使用者確認再補寫分級**。原分支標為「舊文件」而靜默補寫，在新規則下會把「Large 但觸發條件不成立」誤判成 Medium 並將錯誤寫進文件。
...
- **明訂 `**核准 commit**` 的效期到合併為止**：原規則要求合併流程含 squash / rebase / amend / cherry-pick 時，以後續提交把 `**核准 commit**` 回填為合併後仍可達的 SHA。實務上這讓**每個 Medium / Large 與 Small + Medium / High 的 issue 都多出一個提交**，且落在 PR 已合併之後的目標分支上——一個沒人審過的提交。

  追查後發現這個提交沒有讀者：

  | skill | 讀 `**核准 commit**` 的時機 |
  |---|---|
  | `new-issue` | 建檔時寫入 |
  | `review` | 實作期間查核 |
  | `create-pr` | 合併前驗收 |
  | `dev-cycle` | 「完成」收尾——**執行回填本身** |

  三個消費者全在合併前。合併後唯一碰這個欄位的是執行回填的那一步，**回填出來的 SHA 唯一的讀者是回填動作自己**。而「核准了什麼、什麼時候核准」在 `## Gherkin 核准紀錄` 的核准來源與逐 Scenario 日期裡本來就有，Gherkin 原文也一直在 README 裡；SHA 只多回答「核准後有沒有被改」，那個問題隨實作結束一併結束。

  **真正的缺口是終態未定義**：`:230` 已寫成條件式（「合併後**若要**保留可回查性」），但 `:231` 要求「不得留下指向已失效 SHA 的紀錄」。選擇不回填的人因此落在一個規則允許、卻同時禁止其結果的狀態——agent 面對這個缺口只會選唯一有明確做法的那條路，也就是開提交。多餘的提交是缺口的產物，不是規則的本意。

  本版改為宣告作用域而非追新的 SHA：合併後該 SHA 是否可達**不再影響任何 gate**，失效屬**已宣告的預期狀態**而非待修的紀錄缺陷；需要合併後精確指標的專案，在**下次觸及該 issue 目錄的提交中順帶回填**，不為此單獨開提交。驗證對象仍是合併目標分支（`git merge-base --is-ancestor`），驗證不通過時維持原值。

  這也讓規範與自身的「issue 文件 merge 後凍結」（「Agent 工作流程與時序保留」、「常青文件更新責任」兩處）重新一致——原規則一邊說 issue 文件合併後凍結，一邊要求合併後回去改它。

  `dev-cycle` 的「完成」收尾與 `review` 第 5 步同步調整：前者不再要求為回填開提交，後者不再把「核准 commit 已失效」當成 review 期間要處理的情況（該查核的作用域是實作期間，此時核准 commit 始終可達）。

  `rules/AGENTS.md` 與 `rules/AGENTS.zh-TW.md` 的「規格修訂的處理」同步改寫（該節同樣描述了回填要求）。

  **合併前的機制完全未動**：`git diff {核准 commit}..HEAD` 仍是規格修訂的查核手段，仍以 git 作為獨立見證。本版只界定它何時停止適用。
