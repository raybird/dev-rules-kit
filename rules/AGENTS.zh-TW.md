...
3. 持久化核准狀態、日期、來源、完整 Scenario ID 集合與核准 Gherkin 的 SHA-256。計算時將 code fence 內文以 UTF-8 編碼、換行正規化為 LF、移除行尾空白並保留一個結尾 LF；實作前重算，不符時必須重新澄清與核准。
4. 若可用則調用 `test-driven-development`；無論是否安裝，都先建立 BDD Step Definitions 並執行劇本，取得與缺少行為直接相關的失敗證據，再修改 production code。
...
不得只為取得綠燈而刪除、略過、弱化、註解或改寫測試。只有已核准的 Gherkin 行為改變，或能證明測試本身錯誤時才可修改測試；修改契約前須記錄理由並取得核准。

純文件、格式或不可執行任務仍須具備 Gherkin 驗收標準。無法使用自動 runner 時，記錄原因並改用可重複的靜態或手動檢查，不得默默跳過驗證。

Superpowers 是選用的流程增強工具，不是必要依賴。未安裝不得降低任何 gate；缺少核准、紅綠燈證據、獨立審查或驗證時仍必須阻止完成。
