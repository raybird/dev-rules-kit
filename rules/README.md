...
本目錄的規則檔會被複製到各專案的規則設定（`.cursorrules`、`~/.config/opencode/AGENTS.md` 等）並隨該專案演化，因此**哪些章節預期被改、哪些改了會與 skill 衝突，必須看得出來**：

- **標為「本節屬專案客製」的章節**（目前為「語言規範」與「Monorepo 規則」）預期每個專案不同，從本 repo 重新複製時保留專案自己的版本。
- **其餘章節提供通用原則**：目標、範圍、驗證與 issue 流程觸發。詳細核准、測試與 review gate 由下游 `docs/AGENTS.md` 指向的參考檔定義，修改時檢查這些入口與技能一致。
- 專案自行新增的段落，建議以分隔標題隔開並註明「同步時不得覆蓋」，重新複製時才不會被蓋掉。

issue 流程的核心與專案客製檔已分離；`docs/agents/project.md` 保留專案內容，更新器只更新未被本地改寫的核心。規則檔本身的客製仍依上列方式保留。詳細同步方式見 [`docs/AGENTS.md`](../docs/AGENTS.md) 的「客製邊界與同步策略」。
