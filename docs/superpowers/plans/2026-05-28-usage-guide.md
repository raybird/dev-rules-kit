...
**Goal:** 新增 `docs/usage.md`，以情境 walkthrough + 各 skill 快速參考的雙層結構，讓新手與老手都能快速上手這套 kit。

**Architecture:** 單一 `docs/usage.md` 分三章節：快速開始（定位說明）、完整閉環示範（OAuth 情境逐步示範）、各 Skill 快速參考（8 個 skill 快查表）。同步更新 README.md 補上連結。

**Tech Stack:** 純 Markdown，無 build/test。驗證方式為人工閱讀確認內容正確。

---

### Task 1：建立 `docs/usage.md`（快速開始 + 完整閉環示範）

**Files:**
- Create: `docs/usage.md`

- [ ] **Step 1：建立 `docs/usage.md`，寫入快速開始章節與 walkthrough Steps 1–6**

`docs/usage.md` 完整內容如下：
...
閱讀 `docs/usage.md`，確認：
- 三個章節標題完整（使用指南、完整閉環示範、各 Skill 快速參考）
- 六個 Step 都有觸發、AI 產出、關鍵行為三個小節
- 八個 skill 快查表欄位一致（觸發/產出/注意，dev-cycle 例外用查詢/推進/注意）
- review 的循環示範出現在 callout 框內

- [ ] **Step 3：Commit**

```bash
git add docs/usage.md
git commit -m "docs: 新增使用說明文件（情境 walkthrough + skill 快速參考）"
```

---

### Task 2：更新 `README.md` 補上連結

**Files:**
- Modify: `README.md`（第 93–94 行，在「自訂與擴充」步驟後新增步驟）

- [ ] **Step 1：在 README.md 的使用方式章節新增第 3 步**
...
原本的「3. 自訂與擴充」改為「4. 自訂與擴充」。

- [ ] **Step 2：確認修改正確**

確認：
- 新步驟 3 在平台選用之後、自訂與擴充之前
- 連結路徑 `./docs/usage.md` 正確
- 原步驟 3 變為步驟 4，編號連貫
...
```bash
git add README.md
git commit -m "docs: 在使用方式章節補上 usage.md 連結"
```
