...
為了在保障開發正確性的同時避免「小變更、重文件」的繁重負擔，專案的文件產生應依據任務複雜度進行動態評估（分為 Small / Medium / Large 三級），只產出對應的必要文件：

1. **輕量級 (Small) 任務**：
   - **適用場景**：單一檔案的 Bug 修正、極簡單的 feature 實作、文字/日誌/設定微調。
   - **產出檔案**：僅 **`README.md`**（此時需求與實作步驟皆合併寫在 README 內，不另建分析與計畫檔案）。
2. **中量級 (Medium) 任務**：
   - **適用場景**：局部功能的 Feature 開發、不涉及複雜架構調整的重構、多檔案但邏輯直觀的修改。
   - **產出檔案**：**`README.md`** + **`implementation-plan.md`**（將分析併入實作計畫的前置章節中，省略單獨的分析文件）。
3. **重量級 (Large) 任務**：
   - **適用場景**：跨多個模組/服務、新增資料庫 schema、複雜系統遷移、重大架構變更或大型功能模組開發。
   - **產出檔案**：完整四件套：**`README.md`**、**`requirement-analysis.md`**、**`technical-analysis.md`**、**`implementation-plan.md`**。
